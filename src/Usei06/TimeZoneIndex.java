...
/**
 * @author Adriano
 * Índice por time_zone_group e country.
 * - Para cada tzGroup, mantém um AVL de CountryBucket (ordenado por país)
 * - Cada CountryBucket tem um TreeSet<Station> ordenado por nome (e país como tie-break)
 * - Validação de linhas segundo o enunciado
 */
public class TimeZoneIndex {

    /** Grupo -> índice interno (AVL por país + hash para lookup rápido) */
    ...
    private static final class CountryBucket implements Comparable<CountryBucket> {
        ...
        CountryBucket(String country) { this.country = country; }
        @Override public int compareTo(CountryBucket o) { return this.country.compareTo(o.country); }
        @Override public String toString(){ return country + " (" + bucket.size() + ")"; }
    }
    ...
    private static final class GroupIndex {
        ...
        int height() { return countriesAVL.height(); }
        int countries() { return byCountry.size(); }
        Iterable<CountryBucket> inOrderCountries() { return countriesAVL.inOrder(); }
    }
    ...
    public List<Station> byTimeZoneGroup(String tzGroup){
        ...
        if (gi == null) return Collections.emptyList();
    ...
    /**
     * Janela de time zone groups. Junta tudo e ordena por (country ASC, name ASC).
     * Se preferires manter “por grupos”, é só não ordenar aqui e concatenar.
     */
    public List<Station> byTimeZoneWindow(List<String> tzGroups){
        if (tzGroups == null || tzGroups.isEmpty()) return Collections.emptyList();
    ...
    public int height(){
        ...
        int max = -1;
        for (GroupIndex gi : groups.values()) max = Math.max(max, gi.height());
        return max;
    }

    /** Lista de países existentes num grupo (ordenados) – opcional, mas dá jeito. */
    public List<String> countriesInGroup(String tzGroup){
        GroupIndex gi = groups.get(tzGroup);
        if (gi == null) return Collections.emptyList();
        List<String> cs = new ArrayList<>();
        for (CountryBucket cb : gi.inOrderCountries()) cs.add(cb.country);
        return cs;
    }
    ...
    private static void validate(Station s){
        if (s == null) throw new IllegalArgumentException("Station null");

        if (isBlank(s.getName()))            throw new IllegalArgumentException("Invalid name");
        if (isBlank(s.getCountry()))         throw new IllegalArgumentException("Invalid country");
        if (isBlank(s.getTimeZoneGroup()))   throw new IllegalArgumentException("Invalid tz group");
